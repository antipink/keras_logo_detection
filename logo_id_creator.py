...
def create_id(infile, outfile):
            ...
            if (frame_id - 1) in logo_details and key in logo_details[frame_id-1]:  
                ...
                for logo in logo_details[frame_id-1][key]:
                    ...
                    if temp < min_distance:
                        min_distance = temp
                        new_id = logo['new_id']
                if new_id != -1 and min_distance < threshold:
                    row['new_id'] = new_id
                else:
                    row['new_id'] = id_counter
                    id_counter += 1
            else:
                row['new_id'] = id_counter
                id_counter += 1
    ...
    with open(outfile, 'w', newline='') as fout:
        ...
        csvfile.writerow(['frame_id', 'brand', 'location', 'size', 'percentage', 'x', 'y', 'confidence', 'count', 'new_id'])
        ...
        for i in frame_ids:
            ...
            for k, v_list in logo_details[i].items():
                for obj in v_list:
                    new_row = []
                    for col in ['frame_id', 'brand', 'location', 'size', 'percentage', 'x', 'y', 'confidence', 'count', 'new_id']:
