...
class YOLO(object):
    _defaults = {
        "model_path": 'model_data/logoD.h5',
        "anchors_path": 'model_data/logoD_anchors.txt',
        "classes_path": 'model_data/logoD_classes.txt',
        "score" : 0.3,
        "iou" : 0.45,
    ...
    def detect_image_write(self, image, count):
        start = timer()
        if self.model_image_size != (None, None):
            assert self.model_image_size[0]%32 == 0, 'Multiples of 32 required'
            assert self.model_image_size[1]%32 == 0, 'Multiples of 32 required'
...
def detect_video(yolo, video_path, output_path=""):
    ...
    fps = "FPS: ??"
    prev_time = timer()
    with open('output.csv', 'w', newline='') as fid:
        csvfile = csv.writer(fid, delimiter=',')
        csvfile.writerow([col_name.strip() for col_name in 'frame_id, brand, location, size, pecentage, x, y, confidence'.split(',')])
        count = 0
        while True:
            ...
            if return_value:
                ...
                break
            image, frame_rows = yolo.detect_image_write(image, count)
